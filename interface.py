...
class Interface:
    ...
    def _run_query_callback(self):
        ...
        self.first_img_label.set(f"DISABLED {sorted_aqps[0]['disabled_option'].replace('enable_', '')}")
        self._try_show_default_plan(sorted_aqps[1]["plan"], self.second_img)
        self.second_img_label.set(f"DISABLED {sorted_aqps[1]['disabled_option'].replace('enable_', '')}")
