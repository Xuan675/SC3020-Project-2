        ...
        if qep_success:
            ...
            if len(sorted_aqps) > 0:
                try:
                    ...
                    option_name = sorted_aqps[0].get('disabled_option', 'Unknown').replace('enable_', '')
                    self.first_img_label.set(f"AQP 1: DISABLED {option_name}")
                except Exception as e:
            ...
            if len(sorted_aqps) > 1:
                try:
                    ...
                    option_name = sorted_aqps[1].get('disabled_option', 'Unknown').replace('enable_', '')
                    self.second_img_label.set(f"AQP 2: DISABLED {option_name}")
                except Exception as e:
